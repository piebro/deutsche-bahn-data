...
There is a script called `get_eva_list.py` to save the EVAs of all relevant train stations. It doesn't work for all station names, the remaining ones can be looked up here: https://wiki.openstreetmap.org/w/images/c/c2/20141001_IBNR.pdf. There are also muliple EVAs for `Berlin Hbf`. I got only the biggest one. For running the script you need to install `pdfplumber`.
